    ...
    <div>
        <div class="row">
            <div class="col-lg-12">
                ...
                <div v-if="languageLevelId" class="form-group">
        ...
        <div v-if="this.languageLevelId" class="form-group">
            <button class="btn btn-primary" v-on:click="editLanguageLevel()">
                <span>{{ $i('Save') }}</span>
            </button>
        </div>
        <div v-else class="form-group">
            <button class="btn btn-primary" v-on:click="addLanguageLevel()">
                <span>{{ $i('Add') }}</span>
            </button>
        </div>
    </div>
...
<script>
    ...
    export default {
        ...
        watch: {},
        methods: {
            ...mapActions(['getDetailLanguageLevel', 'saveEditLanguageLevel', 'saveAddLanguageLevel', 'getLanguages']),
            getLangs() {
            ...
            getLanguageLevel() {
                if (this.languageLevelId > 0) {
                    this.objData.id = this.languageLevelId;
                    this.getDetailLanguageLevel(this.objData).then(res => {
            ...
            editLanguageLevel() {
                ...
                this.saveEditLanguageLevel(this.objData).then((response) => {
                    if (response.success) {
            ...
            addLanguageLevel() {
                ...
                this.saveAddLanguageLevel(this.objData).then((response) => {
                    if (response.success) {
                        this.$message(this.$i('Successful'));
                        this.$emit('closePopup');
                    }
