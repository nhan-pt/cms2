    ...
    <div class="template-content">
        ...
        <div class="row mt-2">
            <div class="col-md-6 col-lg-3 col-12">
                <el-select v-model="objData.memberId" remote clearable filterable :remote-method="remoteMethod" :placeholder="$i('cms_employer_list_employer')">
        ...
        <div class="row mt-4">
            <div class="col-md-6 col-lg-3 col-12">
                <el-select v-model="objData.provinceId" remote clearable :placeholder="$i('Province')">
                    <el-option v-for="item in listProvinces" :key="item.id" :label="item.name" :value="item.id">
                    </el-option>
                </el-select>
            </div>
            <div class="col-md-6 col-lg-3 col-12">
                <el-select v-model="objData.districtId" remote clearable :placeholder="$i('District')">
                    ...
                    <table class="table table-minwidth fixed-table">
                        ...
                        <tbody>
                            <tr v-if="checkKeyword" v-for="(v, k) in listSearch" :key="k">
                                ...
                                <td>
                                    <div class="td-nowrap" :title="v.branchName">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{v.branchName}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="td-nowrap" :title="v.provinceName + v.districtName + v.townName +  v.address">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{v.provinceName}} {{v.districtName}} {{v.townName}} {{v.address}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="td-nowrap" :title="v.companyName">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{v.companyName}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div v-for="(station,i) in v.stations" :key="i">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{station.stationName}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="nowrap-row">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{formatDate(v.startDate)}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="nowrap-row">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{formatDate(v.endDate)}}</a>
                                    </div>
                                </td>
                                <td>
                                    <div class="td-nowrap text-right" :title="v.salary">
                                        <a href="javascript:;" @click="$router.push('/employerpost/show/' + v.id)">{{formatPrice(v.salary)}}</a>
                                    </div>
                                </td>
                                <td>
                                    <button class="btn__pill" v-for="status in listTab" @click="$router.push('/employerpost/show/' + v.id)" :key="status.id" v-if="status.id == v.status" :class="status.class">{{$i(status.label)}} </button>
                                </td>
                                <td class="btn-action fixed-right-row">
                                    <div class="width-120 nowrap-row">
                                        <button :disabled="v.status==EXPIRED" :class="{'not-allowed':v.status==EXPIRED}" class="btn text-white bg-purple" @click="redirect(v.id)">
...
export default {
    ...
    watch: {
        ...
        'searchLocation'(value) {
            ...
            this.filterId(value, this.employerPostList, 'obj', 'branchName')
        },
        'searchStation'(value) {
            this.searchLocation = ''
            this.searchKey = ''
    ...
    methods: {
        ...mapActions(['updateStatusEmployerPost', 'getListEmployerPost', 'getListEmployerMember', 'searchListDistrict', 'searchListProvince', 'searchListStation', 'getBranch']),
        searchJob() {
            this.objData.branchId = this.objData.branchId=="" ? null:this.objData.branchId
        ...
        getDataBranch() {
            this.objDataBranch.employerId = this.objData.memberId
