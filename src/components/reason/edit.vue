    ...
    <div>
        <div class="row">
            <div class="col-lg-12">
                ...
                <div v-if="reasonId > 0" class="form-group">
        ...
        <div class="form-group">
            <button class="btn btn-primary" v-on:click="addReason()">
                <span v-if="reasonId">{{ $i('Save') }}</span>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...mapActions(['getDetailReason', 'saveEditReason', 'saveAddReason', 'getLanguages']),
            getLangs() {
            ...
            getReason() {
                if (this.reasonId > 0) {
                    this.objData.id = this.reasonId;
                    this.getDetailReason(this.objData).then(res => {
            ...
            addReason() {
                ...
                if (!this.reasonId) {
                    this.saveAddReason(this.objData).then((response) => {
                        if (response.success) {
                    ...
                    this.saveEditReason(this.objData).then((response) => {
                        if (response.success) {
                            this.$message(this.$i('Successful'));
                            this.$emit('closePopup');
                        }
